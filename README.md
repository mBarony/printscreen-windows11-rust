...
- **Esconder, destacar, encurtar**: **ocultar** (`D`) apaga uma região de vez — em mosaico sintético (o padrão) ou cor chapada, e num modo que apaga **só as palavras** reconhecidas, preservando gráficos e layout (melhor-esforço: o que o OCR não achar continua visível); o **holofote** (`O`) escurece o resto da imagem e amplia o que ficou dentro; **remover faixa** (`X`) joga fora uma tira da imagem e junta o que sobrou, levando as anotações junto. Um botão da toolbar põe a captura sobre um **fundo decorativo** com sombra, em quatro variações.

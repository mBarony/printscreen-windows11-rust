...
Aplicação standalone de captura de tela para **Windows 11 (x64)**, escrita em Rust. Um único `rustshot.exe`, sem instalador e sem runtime externo: roda em segundo plano na bandeja do sistema e oferece quatro atalhos globais configuráveis: três de captura e um de reconhecimento de texto e QR code.

> Implementação da [Especificação Técnica v1.0](#especificação) (RustShot v1.11).
...
| **Reconhecer texto ou QR** | `Ctrl+Alt+PrtScr` | Arraste sobre um texto na tela e ele vai para a área de transferência, com as quebras de linha. Se houver um **QR code** na região, o conteúdo dele é copiado no lugar do texto — o QR é tentado primeiro, porque é exato. Nenhuma janela se abre; um aviso no alto da tela mostra o começo do que foi copiado e permite recopiá-lo emendado numa linha só |

- **Editor de anotações**: linha (`L`), seta (`S`), retângulo (`R`), elipse (`E`), mão livre (`F`), marca-texto (`H`), numerador (`N`), conta-gotas (`I`, com `Shift` para a cor do texto e arrasto para a média de uma área), **ocultar** (`D`), **holofote** (`O`), **remover faixa** (`X`), **remover objeto** (`K`), texto (`T`) e **régua** (`U`, mede a distância em pixels da imagem, com uma ponta em cada extremidade e o valor no meio); **Recortar** (`C`) mantém apenas a área arrastada (confirme com `Enter`), levando as anotações junto e podendo ser desfeita; 8 cores + seletor livre, espessura 1–12 px, **traço sólido, tracejado ou pontilhado**, com variante **desenhada à mão**, fonte 12–72 px (`Ctrl+scroll` ajusta a espessura — ou a fonte, com Texto ativo); retângulo e elipse podem sair preenchidos, e o retângulo aceita cantos arredondados; o texto é multilinha (`Ctrl+Enter` confirma) e pode ganhar uma pílula clara de fundo para continuar legível sobre qualquer imagem; as teclas das ferramentas e o papel da roda (traço×zoom) são configuráveis na janela de Configurações; numa janela estreita as configurações descem para uma faixa no rodapé, para nenhum botão cobrir outro; `Alt+H`/`Alt+V` criam guias de alinhamento e `Alt+Shift+G` as limpa; `Ctrl+Shift+C` copia as anotações selecionadas e `Ctrl+V` cola — anotações copiadas do RustShot, ou uma imagem da área de transferência como camada movível; `Ctrl+Z`/`Ctrl+Y` desfaz/refaz; `Shift` restringe a forma (45°/quadrado/círculo) e `Alt` a desenha a partir do centro; a roda do mouse dá zoom (25–400%) e o botão do meio faz pan; `Ctrl+C` copia a imagem anotada para a área de transferência e fecha o editor; `Ctrl+S` salva como arquivo e fecha; `Ctrl+P` **fixa a imagem na tela** como janela sempre no topo; um botão salva um **GIF antes e depois**, alternando entre a captura com e sem as anotações; de outro botão dá para **arrastar a captura anotada para outro programa** como arquivo (Explorer, campo de anexo, área de mensagem); `Esc` descarta (confirmando se houver edições).
- **Esconder, destacar, encurtar**: **ocultar** (`D`) apaga uma região de vez — em mosaico sintético (o padrão) ou cor chapada, e num modo que apaga **só as palavras** reconhecidas, preservando gráficos e layout (melhor-esforço: o que o OCR não achar continua visível); o **holofote** (`O`) escurece o resto da imagem e amplia o que ficou dentro; **remover faixa** (`X`) joga fora uma tira da imagem e junta o que sobrou, levando as anotações junto; **remover objeto** (`K`) apaga um elemento e reconstrói o fundo a partir da borda. Um botão da toolbar põe a captura sobre um **fundo decorativo** com sombra, em quatro variações.
...
- **Bandeja do sistema**: a aplicação não tem janela principal — menu com os três modos, captura com 3 s de atraso, **captura com rolagem** (costura uma página mais alta que a tela), repetir a última região, abrir pasta de capturas, configurações, "Iniciar com o Windows" e Sair.
...
- **Não fala com a rede.** Não há cliente HTTP na árvore de dependências e o programa não abre soquete nenhum — nem para telemetria, nem para atualização, nem para upload. É uma propriedade que você confere na lista de dependências abaixo, e não uma promessa que exige auditar comportamento. Por isso o envio direto para armazenamento em nuvem foi **descartado**: quem precisa disso aponta a pasta de destino para uma pasta sincronizada, ou usa o `rustshot://` para um script fazer o envio depois de salvar.
...
| `qr/` | leitura de QR code: `detecta` (imagem → grade de módulos), `dados` (grade → texto), `galois` (Reed-Solomon), `tabelas` (as constantes do ISO/IEC 18004), `formato` (nível, máscara e mapa de funções) |
...
- **v1.11.4**: os três itens que a revisão tinha deixado de fora. A captura deixou de ser copiada para ir a duas threads no destino "salvar e copiar" — o item tinha sido descartado por um erro de contagem meu, o padrão está em três lugares e não num. E as duas funções grandes viraram o que já eram por dentro: `process_shared` virou a lista das oito etapas que executava, e `canvas::draw` foi de 572 para 173 linhas, despachando para `interact` e `paint_frame`. Nada mudou de comportamento nas duas.
- **v1.11.3**: o resto da [revisão de código](docs/revisao-de-codigo-v1.11.md). Quatro otimizações medidas — o `blend` que chamava `f32::round()` quatro vezes por pixel, o `fill_rect` que superamostrava o miolo do retângulo, o replay que refazia redação e moldura a cada edição, e o agrupamento do QR que era quadrático e travava dez segundos numa captura de textura regular. Mais os dois achados que faltava verificar: uma cor com acento no `config.json` derrubava o programa (real, corrigido), e a amostragem da heurística de formato não colapsa como o relatório dizia (falso — mas o comentário prometia coprimalidade que o código não garantia, e agora garante).
- **v1.11.2**: aplicadas as nove correções que a [revisão de código](docs/revisao-de-codigo-v1.11.md) tinha confirmado e deixado documentadas. A mais grave: a recuperação de sessão **parava de gravar depois de 100 edições**, porque a assinatura de sujeira era o índice do histórico, que trava no teto do log. Corrigindo, apareceram outras três — entre elas o campo de porcentagem, que por renascer em 100% a cada quadro escalava de novo em cima do já escalado, e arrastar até 50% não dava metade.
- **v1.11.1**: o leitor de QR da v1.11.0 não lia nada acima da versão 7 — três defeitos somados no detector, achados na [revisão geral do código](docs/revisao-de-codigo-v1.11.md). O que os deixou passar foi o teste: o caminho da imagem parava na versão 5, e as versões altas eram exercitadas a partir da grade pronta, que pula o detector inteiro. Junto saíram quatro pedaços de código morto, entre eles uma constante que prometia re-rolar o hash do executável e nem chegava ao binário.
- **v1.11.0**: o atalho de reconhecer texto passou a ler também **QR code**. O decodificador é próprio e não trouxe dependência nenhuma: detecta os localizadores na imagem, amostra a grade, desfaz a máscara, desintercala os blocos e corrige com Reed-Solomon. O que prova que ele funciona não é o gerador embutido nos testes — gerador e decodificador escritos juntos erram juntos —, e sim cinco símbolos publicados que o programa lê: três figuras do ISO/IEC 18004 e dois vetores de teste do zxing. Com este item o `nucleo/` fecha inteiro, e com ele os marcos de paridade de anotação e paridade Windows.
- **v1.10.0**: a captura anotada passou a sair do editor **arrastada** para outro programa, como arquivo. E a seleção de região parou de piscar: a janela do overlay nascia visível e vazia porque o winit cria toda janela com `CW_USEDEFAULT` e a mostra **antes** de aplicar tamanho e posição — eram 420 ms de um retângulo branco que saltava e crescia. Agora ela nasce oculta e se exibe no primeiro quadro em que já não pede tamanho novo; a condição é o pedido de tamanho, e não a geometria estar certa, porque janela oculta não recebe `WM_PAINT` e esperar por um quadro que ninguém pediria a deixaria invisível para sempre.
- **v1.9.0**: leva grande de anotação e de captura — padrão de traço (tracejado, pontilhado e desenhado à mão), régua, copiar-e-colar de anotações, colagem de imagem, remoção de objeto e GIF antes-e-depois no editor; colunas preservadas e redação por palavra no OCR; seleção por elemento, captura com rolagem, destino configurável e API por `rustshot://` na captura. Também a decisão de **não implementar upload**: o programa não fala com a rede, e isso passou a estar escrito.
